...
export default function FloatButtons({ data }) {
  ...
  return (
    <div className={styles.wrap}>
      <a href={`tel:${hotlineClean}`} className={styles.btn} aria-label="Gọi điện">
      ...
      <a href={zaloUrl} target="_blank" rel="noopener noreferrer" className={styles.btn} aria-label="Zalo">
        <span style={{ color: '#ffffeb', fontSize: '12px', fontWeight: 600, letterSpacing: '-0.02em' }}>Zalo</span>
      </a>
      <a href={messengerUrl} target="_blank" rel="noopener noreferrer" className={styles.btn} aria-label="Messenger">
