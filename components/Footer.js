...
const defaultStores = [
  { address: '99B Võ Thị Sáu, phường Xuân Hoà, HCM', mapUrl: 'https://maps.google.com' },
    ...
    <footer className={styles.footer}>
      <div className={styles.grid}>
        <div className={styles.left}>
          ...
          <div className={styles.hotline}>HOTLINE: <a href={`tel:${hotlineClean}`}>{hotline}</a></div>
          <div className={styles.storeTitle}>Hệ thống cửa hàng</div>
          <div className={styles.stores}>
            {stores.map((store, i) => (
              <a key={i} href={store.mapUrl || 'https://maps.google.com'} target="_blank" rel="noopener noreferrer">{store.address}</a>
            ))}
          </div>
          <div className={styles.socials}>
            <a href={facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className={styles.socialIcon}>f</a>
            <a href={instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className={styles.socialIcon}>ig</a>
            <a href={tiktok} target="_blank" rel="noopener noreferrer" aria-label="TikTok" className={styles.socialIcon}>tt</a>
          </div>
          <div className={styles.copy}>{copyright}</div>
        </div>
        <div className={styles.links}>
          <Link href="/ve-akina-florist">{fl.about || `VỀ ${siteName}`}</Link>
          <Link href="/san-pham">{fl.shop || 'ĐẶT HOA'}</Link>
          <Link href="/du-an">{fl.project || 'DỰ ÁN'}</Link>
          <Link href="/academy-va-workshop">{fl.workshop || 'WORKSHOP'}</Link>
          <Link href="/chinh-sach">{fl.policy || 'CHÍNH SÁCH'}</Link>
        </div>
      </div>
      <div className={styles.mobileNav}>
        <Link href="/" className={styles.mobileTab}>
          <svg width="19" height="18" viewBox="0 0 19 18" fill="none"><path d="M17.75 13.2C17.75 15.09 17.75 16.03 17.16 16.62C16.58 17.2 15.64 17.2 13.75 17.2H4.75C2.86 17.2 1.92 17.2 1.34 16.62C0.75 16.03 0.75 15.09 0.75 13.2V8.85C0.75 7.9 0.75 7.43 0.95 7.01C1.15 6.6 1.52 6.31 2.27 5.72L6.77 2.16C7.96 1.22 8.56 0.75 9.25 0.75C9.94 0.75 10.54 1.22 11.73 2.16L16.23 5.72C16.98 6.31 17.35 6.6 17.55 7.01C17.75 7.43 17.75 7.9 17.75 8.85V13.2Z" stroke="#BF9675" strokeWidth="1.5"/></svg>
          <span>{ml.home || 'Trang chủ'}</span>
        </Link>
        <Link href="/san-pham" className={styles.mobileTab}>
          ...
          <span>{ml.shop || 'Đặt hoa'}</span>
        </Link>
        <a href={`tel:${hotlineClean}`} className={styles.mobileTab}>
        ...
        <Link href="/academy-va-workshop" className={styles.mobileTab}>
          ...
          <span>{ml.workshop || 'Workshop'}</span>
        </Link>
